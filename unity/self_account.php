...
class AccountOperation
{
    ...
    public function update_account_info($account, $access_token, $platform_id,$cellphone_os_type)
    {
    	...
    	`access_token`='$access_token', `login_time`=NOW(),`cellphone_os_type`=$cellphone_os_type
        ON DUPLICATE KEY UPDATE `session_key`='$session_key', `access_token`='$access_token', `login_time`=NOW(),`cellphone_os_type`=`cellphone_os_type`" ;
    	if ( !mysql_query($query) ) {
    		mysql_close($link);
    ...
    public function get_playerid_info($playerid){
    	...
    	if (mysql_num_rows($result) <= 0) {
    		mysql_close($link);
    		writeLog(get_str_log_prex(__FILE__,__LINE__,__FUNCTION__)." not find playerid,sql:".$select_sql, LOG_NAME::ERROR_LOG_FILE_NAME);
    		return false;
    	}
	...
	//保存每次等了的手机os类型
    public function save_player_cellphone_os_type_every_time($account,$os_type){
    	if (!(0 == $os_type || 1 == $os_type)) {
    		writeLog(get_str_log_prex(__FILE__,__LINE__,__FUNCTION__)." save_player_cellphone_os_type_every_time failure,account:".$account." os_type:".$os_type, LOG_NAME::ERROR_LOG_FILE_NAME);
    		return false;
    	}
    	$link = my_connect_mysql("default");
    	if (!$link) {
    	...
    	$insert_sql = "INSERT INTO `player_cellphone_os_type` SET `account`='".$account."',`cellphone_os_type`=$os_type 
    	ON DUPLICATE KEY UPDATE `cellphone_os_type`=$os_type";
