...
class Guild_level extends CI_Controller {
	...
	public function show($result = array()){
		...
		$this->load->view("guild_level/guild_level_show", $data);
		$this->load->view("templates/footer");
	}
	public function execute($area_id){
		...
		$this->load->model("guild_level_model");
		$result = $this->guild_level_model->search($area_id);
		$this->show(array(
			"result" => $result,
