...
class Vip_level extends CI_Controller {
	...
	public function show($result = array()){
		...
		$this->load->view("vip_level/vip_level_show", $data);
		$this->load->view("templates/footer");
	}
	public function execute($area_id){
		...
		$this->load->model("vip_level_model");
		$result = $this->vip_level_model->search($area_id);
		$this->show(array(
			"result" => $result,
